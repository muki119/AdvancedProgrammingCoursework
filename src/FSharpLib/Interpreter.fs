...
module Interpreter
    ...
    let lexer input = 
        let rec scan input = // recursive function (rec keyword indicates its recursive)
            ...
            | c :: tail when isdigit c -> let (iStr, iVal) = scInt(tail, intVal c) // if digit then call scInt function to get the full number (in case of multiple digits)
    ...
    let parser tList =  // recursive descent parser implementation -- works in BIDMAS order from bottom up 
        ...
        and NR tList = // works from the bottom up -- checks for numbers and parentheses
            ...
            | Lpar :: tail -> match E tail with  // if left parenthesis found then call E on the rest of the list to find the matching right parenthesis
                              | Rpar :: tail -> tail
    ...
    let parseNeval tList = 
        ...
        and Eopt (tList, value) =  // takes a operator and a value
            match tList with
            | Add :: tail -> let (tLst, tval) = T tail // if opperator is addition then call T on the rest of the list which goes to the bottom until a number is found 
        ...
        and NR tList = // works from the bottom up  // the actual evaluation starts here then gets returned up the chain
            ...
            | Lpar :: tail -> let (tLst, tval) = E tail // in the case of a left parenthesis
                              match tLst with 
                              | Rpar :: tail -> (tail, tval) // finds right one 
                              | _ -> raise parseError // raises error otherwise 
            | _ -> raise parseError
        E tList
